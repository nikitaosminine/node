...
export async function firecrawlSearchNews(
  ...
  let res: Response;
  try {
    res = await fetchImpl(`${FIRECRAWL_BASE}/v2/search`, {
      method: "POST",
      headers: {
    ...
    if (err instanceof Error && err.name === "NewsSubrequestBudgetExceededError") throw err;
    if (attempt >= MAX_RETRIES) throw err;
    await sleep(RETRY_BASE_MS * 2 ** (attempt - 1));
    return firecrawlSearchNews(apiKey, options, fetchImpl, attempt + 1);
  }
