...
1. **snapshot** — pulls root `polymarket-curation` runs (portfolio/user attribution +
   ...
   self-contained record of _what the model saw_ and _what it picked_.
...
| **Non-financial leak rate**       | picks whose question matches `NON_FINANCIAL_RE` (sports / entertainment — the same regex used as a category-browse backstop in the Worker). Personalized fanout relies on source-side tag exclusions plus Grok's reject rules, so this metric catches leaks from that path.                                                                                                                                                                                                                                                  | ↓ lower        |
| **Unique-event ratio**            | unique Polymarket events / picks, computed only over runs where **every** enrichable pick has an `event_id` (hallucinated ids can never be enriched, so they don't count against coverage). 100% = maximal diversity. Partially enriched runs are excluded — a partial sample can bias the ratio in either direction — and reported as a count. Requires `event_id` enrichment at snapshot time.                                                                                                                             | ↑ higher       |
| **Runs breaking 2-per-event cap** | share of fully enriched runs where >2 picks share one event, violating the prompt's diversity instruction (server-side dedup later collapses them, so violations silently waste pick slots).                                                                                                                                                                                                                                                                                                                                 | ↓ lower        |
| **Runs missing context**          | runs whose picks exist but whose `grok.score` child trace was missing, unparseable, or parsed back fewer candidate lines than the prompt contained (checked against the parent run's `candidate_count`), so the profile and/or candidate pool are unknown or incomplete. Their picks are excluded from the invalid-id / non-financial / anchored-reason denominators (so blind picks can't dilute those rates) and are not judged — the report warns when this is non-zero.                                                  | ↓ lower        |
| **Judge relevance precision**     | LLM-judge verdicts: relevant picks / judged picks. The judge gets the profile, the market question, and the model's stated reason, and must name a concrete financial mechanism to say "relevant" (rubric in `scripts/eval-curation-lib.mjs:buildJudgePrompt`; geography-overlap, sports, candidacies, and bare "macro uncertainty" are defined as not relevant). Picks whose content is unknown (hallucinated ids, missing-context runs) are counted as _unjudgeable_, not judged. **This is the headline quality number.** | ↑ higher       |
