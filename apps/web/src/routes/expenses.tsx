...
export default function Expenses() {
  ...
  return (
    <div className="mx-auto flex w-full max-w-4xl flex-col gap-5 px-6 py-6 pl-14">
      ...
      {/* Allocation strip (1A-105) + spending heatmap (1A-103) + category mix (1A-106) —
          once there's data. The category mix shares the bottom row with the full
          transaction log (1A-104) once that lands. */}
      {user && rows.length > 0 && (
        <>
      ...
      {rows.length > 0 && (
        <p className="text-xs text-foreground-muted">
          The full transaction log builds on this data next.
        </p>
      )}
