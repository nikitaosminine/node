...
// The phone gutter rides on `w-` rather than `max-w-` so that callers overriding
// `max-w-*` keep it; `max-w-lg` still caps the default size on wide screens.
...
>(({ className, children, hideCloseButton, ...props }, ref) => (
  <DialogPortal>
    ...
    <DialogPrimitive.Content
      ref={ref}
      className={cn(
        // Without a height cap and scroller here, this positions via fixed
        // top-[50%]/translate-y-[-50%] with no max-height, and DialogOverlay is a
        // plain fixed inset-0 with no overflow-y, so nothing in the dialog subtree
        // could scroll and content past the viewport was unreachable.
